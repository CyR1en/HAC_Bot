...
/*  CleverBotQuery API Wrapper - A simple Java API Wrapper for CleverBotQuery!
 *
...
public class CleverBotQuery {
    ...
    /**
     * URL String used for CleverBotQuery connection. <b>Used internally</b>
     */
    ...
    /**
     * Sets the input phrase to be send in the CleverBotQuery query.
     *
     * @param phrase input phrase to be sent to CleverBotQuery
     */
     ...
     *
     * @return String object containing the CleverBotQuery response
     */
     ...
     *
     * @param response CleverBotQuery response
     */
     ...
     * <p>
     * <p>
     * Set to empty ("") in order to start a new conversation with the CleverBotQuery.
     * </p>
     *
     ...
     * <p>
     * <p>
     * CleverBotQuery API queries provide a random number greater than zero and less than one thousand.
     * </p>
     *
     ...
     * <p>
     * This is a unique string used to identify conversation context to the CleverBotQuery API servers.
    ...
    /**
     * Sends request to CleverBotQuery servers. API key and phrase should be set prior to this call
     *
    ...
    public void sendRequest() throws IOException {
        ...
        /* Create JSON Object */
        jsonObject = new JsonParser().parse(inputLine).getAsJsonObject(); // convert to JSON
    ...
    public JsonObject getJsonObject() {
        ...
        json.remove("cs");
        json.remove("interaction_count");
        json.remove("input_other");
        json.remove("input_label");
        json.remove("predicted_input");
        json.remove("accuracy");
        json.remove("errorline");
        json.remove("reaction");
        json.remove("reaction_tone");
        json.remove("emotion");
        json.remove("emotion_tone");
        json.remove("reaction_degree");
        json.remove("emotion_degree");
        json.remove("reaction_values");
        json.remove("emotion_values");
        json.remove("callback");
        json.remove("clever_match");
        json.remove("CSRES30");
        json.remove("output_label");
        json.remove("time_second");
        json.remove("time_minute");
        json.remove("time_year");
        json.remove("time_month");
        json.remove("time_day");
        json.remove("time_hour");
        for (int i = 1; i <= 50; i++) {
            if (json.get("interaction_" + i).getAsString().equalsIgnoreCase(""))
                json.remove("interaction_" + i);
        }
        return jsonObject;
    }
     ...
     * @param key            API key (cleverbot.com/api)
     * @param phrase         input to be sent to CleverBotQuery servers
